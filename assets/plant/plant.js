import {plant,fruit,soil} from './sprites.js';
...
if(canvas){
 ...
 function draw(){
  button.dataset.phase=phase;button.dataset.growth=progress.toFixed(2);
  ...
  if(phase==='grow'||phase==='ripe'){
   ...
   const t=Math.min(time/2,1),ease=t*t*(3-2*t),newBase=landing-(landing-base)*ease;
 ...
 function frame(now){
  ...
  else if(phase==='fall'&&time>3.1){phase='seed';time=0}
